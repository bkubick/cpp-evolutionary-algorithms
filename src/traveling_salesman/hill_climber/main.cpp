...
int main(int argc, char* argv[])
{
    ...
    HillClimberAlgorithm hca = HillClimberAlgorithm(fitness);
    ...
    for (auto algorithm_constant : algorithm_constants) {
        ...
        std::string output_title = "Running Hill Climber Algorithm";
        ...
        std::vector<double> fitnesses = hca.run(population_size, number_of_generations);

        std::string label = population_text + " " + generation_text;

        plt::plot(fitnesses, {{"label", label}});
        plt::title("Hill Climber Algorithm - Total Distance Traveled");
